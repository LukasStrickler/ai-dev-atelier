...
**Command**: `npm run ada::review:task`
...
2. **Run review**: Execute `npm run ada::review:task`
   - CodeRabbit analyzes uncommitted files
   - Review is saved to `.ada/data/reviews/`
3. **Read results**: Run `npm run ada::review:read` to view the latest review
...
2. **Run review**: Execute `npm run ada::review:pr`
...
5. **Finalize**: Run `ada::agent:finalize` and `ada::docs:check` to ensure completeness
6. **Clean up** (optional): Run `npm run ada::review:cleanup` to remove old review files

### Reading Review Results

Use `npm run ada::review:read` to display the most recent review:
...
# Complete workflow
npm run ada::review:task && npm run ada::review:read
