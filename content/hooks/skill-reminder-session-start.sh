#!/bin/bash
# SessionStart hook: Reminds agent to check skills before starting work
# Applies to main agent AND subagents
#
# Input: JSON via stdin (Claude Code / oh-my-opencode format)
# Output: JSON with additionalContext (exit 0)
#
# This hook is non-blocking and injects context at session start.
# The reminder persists until compaction, at which point the
# PreCompact hook re-injects it.
...
cat <<EOF
{
  "hookSpecificOutput": {
    ...
    "additionalContext": $(printf '%s' "$REMINDER" | jq -Rs .)
  }
}
EOF
